...
class PostController extends Controller
{
    ...
    public function create($lang): View
    {
        return view('posts.create', [
            'categories' => $this->categoryRepository->getChildrenOnly(),
        ]);
    }

    public function store($lang, PostStoreRequest $postStoreRequest): RedirectResponse
    {
        $this->postRepository->store($postStoreRequest);

        return redirect()
            ->route('posts.index', ['lang' => $lang])
            ->with('success', 'Пост успешно создан!');
    }

    public function show($lang, Post $post): View
    {
    ...
    public function destroy($lang, Post $post): RedirectResponse
    {
        $this->postRepository->destroy($post);
        return redirect()->route('posts.index', ['lang' => $lang])
